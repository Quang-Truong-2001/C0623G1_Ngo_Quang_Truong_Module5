// 1. Tạo một mảng mới chứa các số lớn hơn 5 từ mảng ban đầu (map).
// 2. Sử dụng arrow function và reduce để tính tổng các phần tử trong mảng.
// 3. Kiểm tra 1 mảng có chứa số V hay không nếu có trả về V không thì trả về "không tìm thấy" (some).
// 4. Kiểm tra 1 mảng tất cả các phần tử trong mảng đó có lớn hơn 0 hay không? (every).
// 5. Tìm phần tử đầu tiên trong mảng lớn hơn 3.
// 6. Sử dụng destructuring với rest parameter để trích xuất phần tử đầu tiên vào biến "first" và các phần tử còn lại vào một mảng mới "rest".
// 7. Sử dụng destructuring để trích xuất các giá trị "name" và "age" từ một mảng chứa các đối tượng "person".
// 8. Sử dụng Rest parameter và Spread operator để tạo một hàm nhận vào danh sách các số và trả về tổng của chúng.
// 9. Sử dụng Rest parameter để nhận vào một danh sách tên và trả về chuỗi định dạng "Welcome, [tên1], [tên2], [tên3], ..." cho tất cả các tên.
// 10. Tạo một đối tượng "book" với thuộc tính "title", "author" và "pages" bằng cách sử dụng Enhanced object literals. Đối tượng "book" cũng có phương thức "displayInfo" để in ra thông tin về sách.


// 1. Tạo một mảng mới chứa các số lớn hơn 5 từ mảng ban đầu (map).
let arr = [-3, 1, 2, 3, 4, 5, 6, 7, 8, 9];

// const a=(item)=>{
//     console.log(item)
// }
// arr.forEach(a);

// const b=(item) =>{
//     return item*2;
// }
//
// let newArr=arr.map(b)
// console.log(newArr)

// map
let newArr=[];
const check=(item)=>{
    if(item>5){
        newArr.push(item)
    }
}
arr.map(check);
console.log(newArr)

// filter
// let newFilter=arr.filter((item)=> item%2==0)
// console.log(newFilter)

// 2. Sử dụng arrow function và reduce để tính tổng các phần tử trong mảng.

let sum=arr.reduce((sum, item)=>(sum+item),0)
console.log(sum);

// 3. Kiểm tra 1 mảng có chứa số V hay không nếu có trả về V không thì trả về "không tìm thấy" (some).

const isHasThree=arr.some(number=> number === 3);
console.log(isHasThree)

// 4. Kiểm tra 1 mảng tất cả các phần tử trong mảng đó có lớn hơn 0 hay không? (every).

const isGreatThanZero=arr.every(number=> number > 0);
console.log(isGreatThanZero)

// 5. Tìm phần tử đầu tiên trong mảng lớn hơn 3.

let count = 0;
arr.forEach(number=>{
    if(count===0){
        if(number>3){
            console.log(number);
            count++;
        }
    }
})

// 6. Sử dụng destructuring với rest parameter để trích xuất phần tử đầu tiên vào biến "first" và các phần tử còn lại vào một mảng mới "rest".

let [first,...rest]=arr;
console.log(first)
console.log(rest)

// 7. Sử dụng destructuring để trích xuất các giá trị "name" và "age" từ một mảng chứa các đối tượng "person".

let arrPerson = [
    {
        name: "A",
        age: 20
    },
    {
        name: "B",
        age: 21
    },
    {
        name: "C",
        age: 22
    },
    {
        name: "D",
        age: 23
    },
]

arrPerson.forEach(person=> {
    let {name, age}=person;
    console.log(name)
    console.log(age)
})

// 8. Sử dụng Rest parameter và Spread operator để tạo một hàm nhận vào danh sách các số và trả về tổng của chúng.

const sum=(...rest)=>{
    let sumNumber=rest.reduce((sumNumber,number)=>(sumNumber+number),0);
    console.log(sumNumber);
}
sum(1,2,3,5)

// 9. Sử dụng Rest parameter để nhận vào một danh sách tên và trả về chuỗi định dạng "Welcome, [tên1], [tên2], [tên3], ..." cho tất cả các tên.

const concatString=(...rest)=>{
    let string='Welcome,'
    rest.forEach((name)=>{
        string+=name+','
    })
    console.log(string)
}
concatString('a','b','c','t')

// 10. Tạo một đối tượng "book" với thuộc tính "title", "author" và "pages" bằng cách sử dụng Enhanced object literals. Đối tượng "book" cũng có phương thức "displayInfo" để in ra thông tin về sách.
let title='hoa';
let author='abc';
let pages=6;

let book={
    title,
    author,
    pages,
    displayInfor(){
        return ' Tiêu đề ' + title + ' Tác giả ' + author +' số trang '+ pages
    }
}
console.log(book.displayInfor())